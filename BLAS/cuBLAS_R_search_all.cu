/*
cuBLAS dense：固定 TN 配置，权重W在右的版本，也就是 列主序的 [K,M]^T * [K,N] = [M,N]，最终是行主序的[N,M]
nvcc -o cuBLAS_R_search_all_fp16 cuBLAS_R_search_all.cu -lcublas && ./cuBLAS_R_search_all_fp16
nvcc -o cuBLAS_R_search_all_int8 cuBLAS_R_search_all.cu -lcublas -DUSE_INT8 && ./cuBLAS_R_search_all_int8
nsys profile --trace=cuda,nvtx,cublas,cudnn --cuda-memory-usage=true --stats=true --force-overwrite true --output=nsys_cublas ./cuBLAS_R_search_all_int8
*/

const char* kCsvFileName = "cuBLAS_R_TN_fp16.csv"; // 可修改的结果文件名
...
int main() {
	...
	//因为cuBLAS要求ld维度是4倍数；而这里的ld是k,k,m，因此m必须是4的倍数
