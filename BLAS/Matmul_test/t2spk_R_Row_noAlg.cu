// 稀疏在右，行主序N/T，不优化算法
// $ nvcc -o t2spk_R_Row_noAlg t2spk_R_Row_noAlg.cu -lcusparseLt && ./t2spk_R_Row_noAlg
// $ nsys profile --trace=cuda,nvtx,cublas,cudnn --cuda-memory-usage=true --stats=true --force-overwrite true --output=detail_res_t2spk_R_Row_noAlg ./t2spk_R_Row_noAlg
    ...
    for (int dim : dimensions) {
        ...
        if (matmul_search) {
            ...
            CHECK_CUSPARSE( cusparseLtMatmulSearch(&handle, &plan, &alpha,      // 句柄，计划，α
                                                dA, dB_compressed, &beta,    // 矩阵A，压缩的B，β  
